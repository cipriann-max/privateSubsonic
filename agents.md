...
- **Auth**: all `/rest/*` handlers go through `authOrError` in `src/subsonic/router.ts`; all `/api/*` handlers (except `auth/status` and `setup`) go through `requireAdmin` in `src/api.ts`. Both call `authenticate` in `src/auth.ts`, which resolves the user from the `users` table and then runs `verifyCredentials`. Never add a route that bypasses this chain (health endpoints live outside `/rest` and `/api`).
- **Users are DB-backed**: `src/users.ts` over the `users` table is the only source of authentication. `ADMIN_USER`/`ADMIN_PASSWORD` were removed — do not reintroduce env-based credentials. Passwords are stored as supplied because Subsonic token auth requires computing `md5(password + salt)`; do not change this to a one-way hash without breaking clients. The first admin is created once via `POST /api/setup` (`createFirstAdmin`, which throws `SetupCompleteError` when any user exists); every later user goes through the admin-only API.
...
- `npm run dev` runs the server with `NODE_ENV=development`, so it does **not** serve `web/dist`; the UI at `http://localhost:5173` (Vite) is the dev frontend and proxies `/rest` and `/api` to the server. Override with `SERVE_WEB=true` to preview the built UI locally.
- Always run the **root** `npm run dev`/`npm run build`, never a workspace script from inside `server/` or `web/`. Running `npm run dev -w server` alone serves no UI; running `npm run start -w server` in the repo serves a **stale** `web/dist`, which looks like the app is broken (old screens, old copy, `/api` calls missing). When in doubt, rebuild.
- Production serves the built UI whenever the built file exists; the SPA fallback excludes `/rest` and `/api`.
